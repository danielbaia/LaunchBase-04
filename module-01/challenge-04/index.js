...
function createTransaction(transaction) {
  user.transations.push(transaction)
...
function getHigherTransactionByType(type) {
  ...
  for (const transaction of user.transations) {
...
function getAverageTransactionValue(user) {
  ...
  const media = total / user.transations.length;
  return media;
}

function getTransactionsCount(user) {
  let amountTransaction = { credit: 0, debit: 0 };

  for (const transaction of user.transations) {
    if (transaction.type == 'credit') {
      amountTransaction.credit = amountTransaction.credit + 1;
    } else {
      amountTransaction.debit = amountTransaction.debit + 1;
    }
  }
